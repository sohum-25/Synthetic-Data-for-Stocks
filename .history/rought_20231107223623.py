...
def fetch_ltp_for_symbol(database_path, fetch_table, max_datetimes, symbol):
    try:
        conn = sqlite3.connect(database_path)
        cursor = conn.cursor()

        max_datetime = max_datetimes.get(symbol, None)
        
        if max_datetime is None:
            # Fetch the maximum Datetime for the symbol if not available in the dictionary
            cursor.execute(f"SELECT MAX(Datetime) FROM {fetch_table} WHERE Symbol = ?", (str(symbol),))

            max_datetime = cursor.fetchone()
            if max_datetime:
                max_datetimes[symbol] = max_datetime[0]

        if max_datetime:
            cursor.execute(f"SELECT LTP FROM {fetch_table} WHERE Symbol = ? AND Datetime = ?", (symbol, max_datetime))
...
def fetch_latest_ltp(symbols, database_path, insert_table, fetch_table):
    max_datetimes = {}  # Dictionary to store maximum Datetime values for symbols
    ...
    while True:
        ...
        if (
            ...
            partial_fetch_ltp = partial(fetch_ltp_for_symbol, database_path, fetch_table, max_datetimes)
            ltp_results = pool.map(partial_fetch_ltp, symbols)
            pool.close()
            pool.join()
