...
def run_tick_generator(config):
    ...
    for _ in range(700):  # Simulate 20 ticks for each instance
        tick = next(ticker)
        print(f"Tick Data for {time.strftime('%H:%M:%S', time.localtime())} {symbol}: {tick}")

# API route for subscribing to tick data via WebSocket
