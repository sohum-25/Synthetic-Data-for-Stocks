...
def fetch_ltp_for_symbol(database_path, fetch_table, symbol, max_datetimes):
    try:
...
def fetch_latest_ltp(symbols, database_path, insert_table, fetch_table):
    ...
    while True:
        ...
        if (
            ...
            ltp_results = pool.map(fetch_ltp_for_symbol, [(database_path, fetch_table, symbol, max_datetimes) for symbol in symbols])
            pool.close()
            pool.join()
