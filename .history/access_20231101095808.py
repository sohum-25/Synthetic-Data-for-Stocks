        ...
        if current_minute % 2 == 1 and current_second == 0:
            try:
                ...
                for symbol in symbols:
                    ...
                    insert_query = f"INSERT INTO {insert_table} (timestamp, {symbol}) VALUES (?, ?)"
                    
                    # Retrieve the latest LTP for each specified symbol based on the maximum timestamp
                    cursor.execute(f"SELECT ltp FROM {fetch_table} WHERE symbol = ? AND timestamp = (SELECT MAX(timestamp) FROM {fetch_table} WHERE symbol = ?)", (symbol, symbol))
...
symbol_sets = [
    ['NSE:APOLLOHOSP-EQ', 'NSE:BANKBARODA-EQ'],
    ['NSE:BPCL-EQ', 'NSE:BRITANNIA-EQ'],
    ['NSE:CANBK-EQ', 'NSE:DIVISLAB-EQ'],
    ['NSE:GRASIM-EQ', 'NSE:HEROMOTOCO-EQ'],
    ['NSE:HINDALCO-EQ', 'NSE:INDUSINDBK-EQ'],
    ['NSE:IOC-EQ', 'NSE:JSWSTEEL-EQ'],
    ['NSE:ONGC-EQ', 'NSE:RELIANCE-EQ'],
    ['NSE:SBILIFE-EQ', 'NSE:SBIN-EQ'],
    ['NSE:SHREECEM-EQ', 'NSE:SHRIRAMFIN-EQ'],
